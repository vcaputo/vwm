 ...
 *
 * This exists to decouple the rendering needs of charts.c/vmon.c from Xlib
 * for enabling headless use of vmon in embedded/server circumstances.
 *
 * Rather than exploring serverless Xlib implementations which render in-process,
 * embracing something like cairo, or worse - creating a new generic rendering
 * library, a very targeted approach has been taken here catering specifically
 * to the requirements of charts.c.
 *
 ...
 * Render based charts usage utilizes several full-color full-size Picture objects
 * for the various layers used to compose the charts.
 *
 ...
 *
 * Imagine for instance if headless vcr allocated a single byte array dimensioned
 * by the chart's dimensions, to represent eight layers in the bit planes of the bytes.
 * This exploits the fact that chart layers contain monochromatic coverage information
 * for the pixel.  Turning these layers into color renderings could use a simple palette
 * lookup to produce the appropriate blended colors given the combination of bits set in
 * the various layers.
 *
 * So every layer would have a color associated with it, used when compositing the rendered
 * chart from the layers.  But the actual layer maintenance would simply be setting/unsetting
 * the appropriate bit in the affected pixel positions.
 */
...
typedef struct vcr_t {
	 ...
	 */
	int		*heirarchy_end_ptr;			/* pointer to row where the process heirarchy currently ends */
...
static vcr_backend_t * vcr_backend_xlib_setup(vcr_backend_t *vbe, vwm_xserver_t *xserver)
{
	...
	/* TODO: error handling for all this junk */
...
/* returns NULL on failure, freeing vcr.  Or vcr on success, fully setup for the given backend. */
static vcr_backend_t * vcr_backend_mem_setup(vcr_backend_t *vbe)
{
	assert(vbe);
...
vcr_dest_t * vcr_dest_free(vcr_dest_t *dest)
{
	if (dest) {
		...
		case VCR_DEST_TYPE_PNG:
			 ...
			 * to it.  But maybe it'd be better to just take ownership of it here.
			 */
 ...
 */
vcr_t * vcr_new(vcr_backend_t *vbe, int *heirarchy_end_ptr, int *snowflakes_cnt_ptr)
{
	vcr_t	*vcr;

	assert(vbe);
	assert(heirarchy_end_ptr);
	...
	vcr->backend = vbe;
	vcr->heirarchy_end_ptr = heirarchy_end_ptr;
...
static void vcr_free_xlib_internal(vcr_t *vcr)
{
	...
	XRenderFreePicture(xserver->display, vcr->xlib.tmp_picture);
...
int vcr_resize_visible(vcr_t *vcr, int width, int height)
{
	...
	case VCR_BACKEND_TYPE_XLIB: {
		...
		vcr->xlib.tmp_picture = create_picture(xserver, vcr->width, VCR_ROW_HEIGHT, CHART_MASK_DEPTH, 0, NULL, NULL);
...
void vcr_draw_text(vcr_t *vcr, vcr_layer_t layer, int x, int row, const char **strs, int n_strs, int *res_width)
{
	assert(vcr);
	assert(vcr->backend);
	...
	case VCR_BACKEND_TYPE_XLIB: {
		...
		for (int i = 0; i < n_strs; i++) {
			items[i].nchars = strlen(strs[i]);
			items[i].chars = (char *)strs[i];
			items[i].delta = 4;
			items[i].font = None;
		}
...
/* draw an arbitrary line into the given layer, on the xlib backend this only works
 * on the VCR_LAYER_TEXT layer, so it's just asserted to only go there for now.. which
 * is fine for the existing callers.
 */
void vcr_draw_line(vcr_t *vcr, vcr_layer_t layer, int x1, int y1, int x2, int y2)
{
	assert(vcr);
	assert(vcr->backend);
...
/* copy what's below a given row up the specified amount within the same layer */
void vcr_shift_below_row_up(vcr_t *vcr, int row, vcr_layer_t layer, int rows)
{
	assert(vcr);
	assert(vcr->backend);
	assert(layer >= 0 && layer < VCR_LAYER_CNT);
	...
	case VCR_BACKEND_TYPE_XLIB: {
		...
		XRenderComposite(xserver->display, PictOpSrc, *layers[layer], None, *layers[layer],
			0, (rows + row) * VCR_ROW_HEIGHT,						/* src */
			0, 0,										/* mask */
			0, row * VCR_ROW_HEIGHT,							/* dest */
			vcr->width, (rows + *(vcr->heirarchy_end_ptr)) * VCR_ROW_HEIGHT - (rows + row) * VCR_ROW_HEIGHT);	/* dimensions */
...
/* copy what's below a given row down the specified amount within the same layer */
void vcr_shift_below_row_down(vcr_t *vcr, int row, vcr_layer_t layer, int rows)
{
	assert(vcr);
	assert(vcr->backend);
	assert(layer >= 0 && layer < VCR_LAYER_CNT);
	...
	case VCR_BACKEND_TYPE_XLIB: {
		...
		XRenderComposite(xserver->display, PictOpSrc, *layers[layer], None, *layers[layer],
			...
			0, 0,								/* mask */
			0, (row + rows) * VCR_ROW_HEIGHT,				/* dest */
			vcr->width, vcr->height - (rows + row) * VCR_ROW_HEIGHT);	/* dimensions */
		break;
	}
#endif
...
void vcr_stash_row(vcr_t *vcr, vcr_layer_t layer, int row)
{
	...
	case VCR_BACKEND_TYPE_XLIB: {
		...
		XRenderComposite(xserver->display, PictOpSrc, *layers[layer], None, vcr->xlib.tmp_picture,
			0, row * VCR_ROW_HEIGHT,	/* src */
			0, 0,				/* mask */
			0, 0,				/* dest */
...
void vcr_unstash_row(vcr_t *vcr, vcr_layer_t layer, int row)
{
	...
	case VCR_BACKEND_TYPE_XLIB: {
		...
		XRenderComposite(xserver->display, PictOpSrc, vcr->xlib.tmp_picture, None, *layers[layer],
			0, 0,				/* src */
			0, 0,				/* mask */
			0, row * VCR_ROW_HEIGHT,	/* dest */
			vcr->width, VCR_ROW_HEIGHT);	/* dimensions */
 ...
 * by having a abs(delta)>1 and show them as blanks in the graph or something like that. TODO FIXME
 */
...
static int vcr_composed_height(vcr_t *vcr)
{
	...
	return MIN((*(vcr->heirarchy_end_ptr) + snowflakes) * VCR_ROW_HEIGHT, vcr->visible_height);
}
...
int vcr_compose(vcr_t *vcr)
{
	...
	case VCR_BACKEND_TYPE_XLIB: {
		...
		XRenderComposite(xserver->display, PictOpOver, vcr->backend->xlib.text_fill, vcr->xlib.text_picture, vcr->xlib.picture,
			...
			0, 0,
			0, 0,
			vcr->visible_width, (*(vcr->heirarchy_end_ptr) * VCR_ROW_HEIGHT));

		XRenderComposite(xserver->display, PictOpOver, vcr->backend->xlib.snowflakes_text_fill, vcr->xlib.text_picture, vcr->xlib.picture,
			0, 0,
			0, *(vcr->heirarchy_end_ptr) * VCR_ROW_HEIGHT,
			0, *(vcr->heirarchy_end_ptr) * VCR_ROW_HEIGHT,
			vcr->visible_width, height - (*(vcr->heirarchy_end_ptr) * VCR_ROW_HEIGHT));
		break;
	}
#endif
...
static void vwm_chart_render_as_pixmap(vwm_charts_t *charts, vwm_chart_t *chart, const XRenderColor *bg_color, Pixmap *res_pixmap)
{
	...
	dest = create_picture_fill(charts, chart->width, vcr_composed_height(chart->vcr), 32, 0, NULL, bg_color, res_pixmap);
	vwm_chart_render(charts, chart, PictOpOver, dest, 0, 0, chart->width, vcr_composed_height(chart->vcr));
	XRenderFreePicture(charts->xserver->display, dest);
}


static void vwm_chart_render_as_ximage(vwm_charts_t *charts, vwm_chart_t *chart, const XRenderColor *bg_color, XImage **res_ximage)
{
	Pixmap	dest_pixmap;
...
int vcr_present(vcr_t *vcr, vcr_present_op_t op, vcr_dest_t *dest, int x, int y, int width, int height)
{
	...
	case VCR_BACKEND_TYPE_XLIB:
		...
		case VCR_DEST_TYPE_XWINDOW: {
			...
			XRenderComposite(xserver->display, PictOpSrc, vcr->xlib.picture, None, dest->xwindow.picture,
	...
	case VCR_BACKEND_TYPE_MEM:
		...
		case VCR_DEST_TYPE_XPICTURE:
			...
			VWM_ERROR("vcr_present mem->x{window,picture} unsupported");
