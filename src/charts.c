...
static void snowflake_row(vwm_charts_t *charts, vwm_chart_t *chart, vcr_layer_t layer, int copy, int row)
{
	VWM_TRACE("pid=%i chart=%p row=%i copy=%i heirarhcy_end=%i", chart->monitor->pid, chart, row, copy, chart->heirarchy_end);

	if (copy)
		vcr_stash_row(chart->vcr, layer, row);

	vcr_shift_below_row_up(chart->vcr, row, layer, 1);

	if (copy) {
		vcr_unstash_row(chart->vcr, layer, chart->heirarchy_end);
	} else {
		vcr_clear_row(chart->vcr, layer, chart->heirarchy_end, -1, -1);
	}
}
...
static void allocate_row(vwm_charts_t *charts, vwm_chart_t *chart, vcr_layer_t layer, int row)
{
	VWM_TRACE("pid=%i chart=%p row=%i", chart->monitor->pid, chart, row);

	vcr_shift_below_row_down(chart->vcr, row, layer, 1);
	vcr_clear_row(chart->vcr, layer, row, -1, -1);
}
...
static void proc_argv2strs(const vmon_proc_t *proc, const char **strs, int max_strs, int *res_n_strs)
{
	int	nr = 0;
	...
	if (!proc->is_thread) { /* suppress the argv for threads */
		for (int i = 1; nr < max_strs && i < ((vmon_proc_stat_t *)proc->stores[VMON_STORE_PROC_STAT])->argc; nr++, i++) {
			strs[nr] = ((vmon_proc_stat_t *)proc->stores[VMON_STORE_PROC_STAT])->argv[i];
		}
	}
...
{
	const char	*strs[VCR_DRAW_TEXT_N_STRS_MAX];
	int		n_strs;
	...
	if (proc != chart->monitor) {
		...
		for (sub = 1, ancestor = proc->parent; ancestor && ancestor != chart->monitor; ancestor = ancestor->parent, sub++) {
			...
			if (proc_has_subsequent_siblings(&charts->vmon, ancestor))
				vcr_draw_line(chart->vcr, VCR_LAYER_TEXT,
...
static void draw_chart_rest(vwm_charts_t *charts, vwm_chart_t *chart, vmon_proc_t *proc, int *depth, int *row)
{
	...
	if (proc->is_stale) {
		...
		snowflake_row(charts, chart, VCR_LAYER_GRAPHA, 1, (*row));
		snowflake_row(charts, chart, VCR_LAYER_GRAPHB, 1, (*row));
		snowflake_row(charts, chart, VCR_LAYER_TEXT, 0, (*row));
		snowflake_row(charts, chart, VCR_LAYER_SHADOW, 0, (*row));
		...
		allocate_row(charts, chart, VCR_LAYER_GRAPHA, (*row));
		allocate_row(charts, chart, VCR_LAYER_GRAPHB, (*row));
		allocate_row(charts, chart, VCR_LAYER_TEXT, (*row));
		allocate_row(charts, chart, VCR_LAYER_SHADOW, (*row));
...
vwm_chart_t * vwm_chart_create(vwm_charts_t *charts, int pid, int width, int height, const char *name)
{
	...
	chart->vcr = vcr_new(charts->vcr_backend, &chart->heirarchy_end, &chart->snowflakes_cnt);
