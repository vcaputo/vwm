...
#define CHART_NUM_FIXED_HEADER_ROWS	4				/* number of rows @ top before the hierarchy: { IOWait/Idle, User/Sys, IRQ/SoftIRQ, Adherence (behind column header labels) } */
...
typedef struct _vwm_charts_t {
	...
	unsigned long long			last_user, last_system, user_delta, system_delta;
 ...
 * as well as the multi-row use adding per-proc memory rows will start doing - there will need to be a way to
 * for instance get the _TREE column into the memory row for continuity purposes (though it will likely need to be
 * augmented slightly in the sub-row or whatever that becomes) as there won't be any Argv to point at in the memory
 * row following the CPU row.
 */
...
static void sample_callback(vmon_t *vmon, void *arg)
{
	...
	charts->user_delta = sys_stat->user - charts->last_user;
	charts->system_delta = sys_stat->system - charts->last_system;
	...
	for (int i = 0, left = 0, right = 0; i < CHART_MAX_COLUMNS; i++) {
		...
		case VWM_COLUMN_PROC_ARGV: { /* print the process' argv */
			if (heading) {
				...
				print_argv(charts, chart, left /* FIXME: consider rc->side */, row, proc, &width);
				if (width > c->width) {
					c->width = width;
					chart->redraw_needed++;
				}
		...
		if (str_len) {
			...
			/* get xpos to the left edge of the column WRT c->width and rc->side */
			switch (rc->side) {
			case VWM_SIDE_LEFT:
				xpos = left;
				break;
		...
		if (advance) {
			left += (rc->side == VWM_SIDE_LEFT) * (c->width + VCR_ROW_HEIGHT / 2);
			right += (rc->side == VWM_SIDE_RIGHT) * (c->width + VCR_ROW_HEIGHT / 2);
		}
	}
}
...
vwm_chart_t * vwm_chart_create(vwm_charts_t *charts, int pid, int width, int height, const char *name)
{
	...
	chart->proc_cpu_columns[1] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_USER };
	chart->proc_cpu_columns[2] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_SYS };
	chart->proc_cpu_columns[3] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_WALL };
	chart->proc_cpu_columns[4] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_TREE };
	chart->proc_cpu_columns[5] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_ARGV };
	chart->proc_cpu_columns[6] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_STATE };
	...
	chart->snowflake_cpu_columns[1] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_PPID };
	chart->snowflake_cpu_columns[2] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_USER };
	chart->snowflake_cpu_columns[3] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_SYS };
	chart->snowflake_cpu_columns[4] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_WALL };
	chart->snowflake_cpu_columns[5] = (vwm_column_t){ .enabled = 1, .type = VWM_COLUMN_PROC_ARGV };
	...
	chart->snowflake_cpu_row_columns[4] = (vwm_row_column_t){ .column = &chart->snowflake_cpu_columns[4], .side = VWM_SIDE_LEFT, .justify = VWM_JUSTIFY_RIGHT };
	chart->snowflake_cpu_row_columns[5] = (vwm_row_column_t){ .column = &chart->snowflake_cpu_columns[5], .side = VWM_SIDE_LEFT, .justify = VWM_JUSTIFY_LEFT };
...
int vwm_charts_update(vwm_charts_t *charts, int *desired_delay_us)
{
	...
	if (!charts->primed ||
		...
		if ((sys_stat = charts->vmon.stores[VMON_STORE_SYS_STAT])) {
			...
			charts->last_user = sys_stat->user;
			charts->last_system = sys_stat->system;
